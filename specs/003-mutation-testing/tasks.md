...
- [ ] T009 Verify the resolved scope matches the plan's measured figures: the set of files Stryker reports as mutated MUST be **32 files** totalling **3383 lines**, distributed `src/capacity/domain` 9, `src/capacity/application` 14, `src/shared/money` 4, `src/shared/result` 2, `src/treasury/handlers` 2, `src/treasury/retry` 1. A different count means a glob is wrong — reconcile before measuring anything.
...
- [ ] T010 [US1] Confirm the precondition before measuring: run `npm run test:unit` and require it green. A run against an already-red suite reports a meaningless score, and the spec's edge case requires stopping and reporting the broken suite rather than a number. Record the observed suite size — 29 suites / 287 tests / ~6.0 s as measured on 2026-09-21 — and investigate any material divergence before continuing.
