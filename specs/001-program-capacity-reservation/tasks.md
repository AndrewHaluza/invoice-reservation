...
- [ ] T104 Extend per-organisation rate limiting so one organisation cannot monopolise contention on a single program — the current `read`/`write` throttlers in `src/auth/throttler.config.ts` bound total per-org request volume only — or record the omission as an explicit trade-off in `docs/ASSUMPTIONS.md` per FR-033 (partial)
- [ ] T105 Make the recovery hold observable: `RecoveryDetectionService` in `src/capacity/application/recovery-detection.service.ts` sets `position_verified = FALSE` with no log line, metric, or alert, so a program held pending a fresh snapshot is invisible to operators — emit a gauge alongside `over_limit_programs`/`investigation_required_programs` in `src/observability/metrics.ts` and log each flagged program per FR-020 and the "stream aged out beyond the restored position" edge case (partial)
- [ ] T106 Exercise the repeat-stability clause of the concurrency guarantee — `test/integration/concurrency.spec.ts` asserts one 1,000-attempt run, not "100% of 20 consecutive runs" — by adding a repeated-run harness (or recording the deviation and its cost in `docs/ASSUMPTIONS.md`) per SC-001 (partial)
